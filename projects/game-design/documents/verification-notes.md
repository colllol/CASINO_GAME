...
| No unset value was given a default | Scanned every parameter table added in Phase 1 | 10 parameters unset across D17, D18, and D22; all specified fail-closed, none defaulted |
...
the robbery transfer magnitude (D17) and every jackpot odd (D22) are unset by Owner gate, so no
RTP figure exists for the fourth game and no expected-loss figure exists for robbery. The slots
and roulette figures above remain the only enumerated house-edge numbers in the project.
...
| T40 | Initiating a robbery drops the initiator's immunity; an immune player gets no free first strike | RB12 |
| T41 | The server refuses to enable robbery while any D17 or D18 parameter is unset | RB13, I14, PA19 |
...
| ID | Assertion | Invariant |
| --- | --- | --- |
| T44 | The jackpot machine refuses every wager while any D22 parameter is unset | G12, I14 |
...
T44 and T48 are the two that cannot be written until D22 is answered. T45 through T47, T50, and
T51 are writable now against a test configuration, and T47 should exist before the first cabinet
is playable because a double-paid pool is nearly invisible in play.
...
the project's single assertion index. And T44 and T48 cannot be written at all until Owner
decision D22 supplies the jackpot odds, which is the intended consequence of this surface
...
| --- | --- | --- |
| D16 | Confirm Decision 0001 (engine and service stack) | The adapter's eventual replacement target; T65's mapping claim |
| D17 | Robbery carried-cash share and per-robbery cap | Robbery cannot be enabled (T41); no expected-loss figure exists |
| D18 | Per-aggressor robbery cooldown | Robbery cannot be enabled (T41) |
...
| D21 | Amend economy constraint C2 to "no **voluntary** transfer" | T43, and Decision 0002 guarantee 2 |
| D22 | Jackpot outcome space, paytable, hit odds, contribution share, seed, fixed stake | T44 and T48 cannot be written; no RTP figure exists for the fourth game |
| D23 | Confirm ten concurrent players | T75-T77 |
| D24 | Confirm Windows-only | T78 |
| D25 | Confirm the local Phase 1 persistence adapter | T52-T74 |
...
D17 and D22 are the two that no amount of further verification can work around: they are loss
percentages and odds, and this surface is forbidden to invent them. Everything they gate is
specified to fail closed rather than to default, so an unanswered gate produces a refusal to
start, never a silently wrong number.
